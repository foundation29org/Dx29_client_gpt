...
/**
 * Script para reemplazar "DxGPT" por "{{TENANT_NAME}}" en archivos de traducción
 * 
 * Uso: node scripts/replace-dxgpt-in-translations.js [--dry-run] [--backup]
 * 
 * Opciones:
 *   --dry-run    Solo muestra qué cambios se harían, sin modificar archivos
 *   --backup     Crea una copia de seguridad antes de modificar
 * 
 * Ejemplo: node scripts/replace-dxgpt-in-translations.js --dry-run
 */
...
function processTranslationFile(filePath, options = {}) {
    ...
    try {
        ...
        // Reemplazar DxGPT por {{TENANT_NAME}} en valores JSON
        ...
        content = content.replace(/: "([^"]*DxGPT[^"]*)"/g, (match, value) => {
            ...
            const newValue = value.replace(/DxGPT/g, '{{TENANT_NAME}}');
            changes++;
...
function main() {
    ...
    console.log('🔄 Script de reemplazo de DxGPT en archivos de traducción');
    ...
    if (dryRun && totalChanges > 0) {
        ...
        console.log('   node scripts/replace-dxgpt-in-translations.js');
        console.log('\n💡 Para crear backups y aplicar cambios:');
        console.log('   node scripts/replace-dxgpt-in-translations.js --backup');
    }
