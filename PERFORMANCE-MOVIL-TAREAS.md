...
**Corrección de inicialización de GA (19/08, tarde):** al validar en Chrome, Edge y Firefox se descargaba `gtag.js`, pero no aparecía ninguna petición a `google-analytics.com/g/collect`. La causa era que `dataLayer`, `gtag('js', ...)` y los comandos `gtag('config', ...)` se creaban dentro de `script.onload`, cuando `gtag.js` ya se había ejecutado. Se ha restaurado el orden del snippet oficial: preparar `dataLayer` y encolar toda la configuración antes de insertar el script; `onload` ahora solo marca el servicio como listo y vacía la cola de eventos. Se mantienen el diferido de 2 s, la guarda contra duplicados y la cola. Añadida además una guarda `googleTagsConfigured` para que un reintento tras error de red no duplique configuraciones ni conversiones. Pendiente confirmar tras desplegar que aparece `g/collect` con `tid=G-2FZQ49SRWY` y estado 204.

**Prueba controlada de cobertura de GA (19/08, noche):** ante la sospecha de que el diferido pudiera omitir visitas muy breves, se ha añadido `DEFER_GOOGLE_TAGS = false`. Durante esta prueba GA/Ads se cargan inmediatamente después del consentimiento, pero se mantienen el orden oficial de inicialización, deduplicación, cola y Hotjar desactivado. No se restauró el archivo temporal anterior porque mantenía el mismo diferido de 2 s, reintroducía la inicialización defectuosa dentro de `onload` y reactivaba Hotjar, por lo que habría cambiado tres variables a la vez. Para revertir esta prueba basta poner `DEFER_GOOGLE_TAGS = true`.
